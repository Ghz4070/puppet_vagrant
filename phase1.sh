...
if [ ! -f /vagrant/.env ]; then
    echo >&2 "ERROR: unable to find /vagrant/.env file"
    exit 1
fi
if ! grep -q '^USER_EMAIL=' /vagrant/.env; then
    echo >&2 "ERROR: unable to find USER_EMAIL key in /vagrant/.env file"
    exit 1
fi
eval "$(grep '^USER_EMAIL=' /vagrant/.env)"

if ! grep -q '^USER_NAME=' /vagrant/.env; then
    echo >&2 "ERROR: unable to find USER_NAME key in /vagrant/.env file"
    exit 1
fi
eval "$(grep '^USER_NAME=' /vagrant/.env)"

if ! grep -q '^GIT_HOST=' /vagrant/.env; then
    echo >&2 "ERROR: unable to find GIT_HOST key in /vagrant/.env file"
    exit 1
fi
eval "$(grep '^GIT_HOST=' /vagrant/.env)"

if ! grep -q '^GIT_REPOSITORY=' /vagrant/.env; then
    echo >&2 "ERROR: unable to find GIT_REPOSITORY key in /vagrant/.env file"
    exit 1
fi
...
if [ ! -f /vagrant/githosting_rsa ]; then
    echo >&2 "ERROR: unable to find /vagrant/githosting_rsa keyfile"
    exit 1
fi
if [ ! -f /vagrant/githosting_rsa.pub ]; then
    echo >&2 "ERROR: unable to find /vagrant/githosting_rsa.pub keyfile"
    exit 1
fi
